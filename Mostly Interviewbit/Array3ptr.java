...
public class Array3ptr {
	...
	public static int minimize(final Integer[] array1, final Integer[] array2, final Integer[] array3) {
		
		Integer minVal = Integer.MAX_VALUE;
		//int i=0, j=0, k=0;
		int indices[]= new int[3];
		/*Integer[] array1 = new Integer[a.size()];
		array1 = a.toArray(array1);
		
		Integer[] array2 = new Integer[b.size()];
		array2 = b.toArray(array2);
		
		Integer[] array3 = new Integer[c.size()];
		array3 = c.toArray(array3);*/
		
		for(Integer i :array1){
			int closestInB = getClosestK(array2, i);
			int closestInC = getClosestK(array3, i);
			int currMin = Math.max(Math.abs(i - closestInB),Math.max(Math.abs(closestInB - closestInC), Math.abs(closestInC - i)));
			if (currMin < minVal) {
				minVal = currMin;
				indices[0] = Arrays.binarySearch(array1, i);
				indices[1] = Arrays.binarySearch(array2, closestInB);
				indices[2] = Arrays.binarySearch(array3, closestInC);
				
			}
		}
		
		for(Integer i :array2){
			int closestInA = getClosestK(array1, i);
			int closestInC = getClosestK(array3, i);
			int currMin = Math.max(Math.abs(closestInA - i),Math.max(Math.abs(i - closestInC), Math.abs(closestInC - closestInA)));
			if (currMin < minVal) {
				minVal = currMin;
				indices[0] = Arrays.binarySearch(array1, closestInA);
				indices[1] = Arrays.binarySearch(array2, i);
				indices[2] = Arrays.binarySearch(array3, closestInC);
		...
		for(Integer i :array3){
			int closestInA = getClosestK(array1, i);
			int closestInB = getClosestK(array2, i);
			int currMin = Math.max(Math.abs(closestInA - closestInB),Math.max(Math.abs(closestInB - i), Math.abs(i - closestInA)));
			if (currMin < minVal) {
				minVal = currMin;
				indices[0] = Arrays.binarySearch(array1, closestInA);
				indices[1] = Arrays.binarySearch(array2, closestInB);
				indices[2] = Arrays.binarySearch(array3, i);
		...
		/*for(i=0,j=0,k=0;i<A.size() && j<B.size() && k<C.size();) {
			...
			int currB = B.get(j);
			int currC = C.get(k);
			int currMin = Math.max(currA - currB,Math.max(currB - currC, currC - currA));
			if (minVal == null ||currMin < minVal) {
				minVal = currMin;
			}
			boolean canIncI = i != a.length-1;
			boolean canIncJ = j != b.length-1;
			boolean canIncK = k != c.length-1;
			if ((i<A.size()) && !canIncJ && !canIncK)
				break;
			
		
			if((i<A.size()) && (j==B.size() || currA < currB) && (k==C.size() || currA<currC))
				i++;
			if (canIncI && (!canIncJ || a[i] < b[j]) 
				&& (!canIncK || a[i] < c[k]))
				++i;
			
			else if(j<B.size() && (k==C.size() || currB<currC))
				j++;
		...
		System.out.println(indices[0]+"  "+indices[1]+"  "+indices[2]+"  "+"  "+minVal);
		return minVal;
	}
	
	public static int getClosestK(Integer[] array1, int x) {
	    ...
	    */
	    while (low < high) {
	        int mid = (low + high) / 2;
	        assert(mid < high);
	        int d1 = Math.abs(array1[mid  ] - x);
	        int d2 = Math.abs(array1[mid+1] - x);
	        if (d2 <= d1)
	        {
	            low = mid+1;
	        }
